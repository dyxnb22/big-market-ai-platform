...
Status anchor: Phase 4-D/E/F complete (tag `phase-4-strategy-read-adapter-boundary`). Phase 3 complete. Phase 4 complete. Phase 5-A orchestration map complete (tag `phase-5-activity-draw-orchestration-map`). Phase 5-B draw-command boundary design doc complete. Phase 5-C account/quota port re-verification complete. Phase 5-D local strategy decision port introduced (tag `phase-5-strategy-decision-port-boundary`). Phase 5-E local award fulfillment port introduced (tag `phase-5-award-fulfillment-port-boundary`). Phase 5-F activity-service dark-launch scaffold introduced (tag `phase-5-activity-service-dark-launch-scaffold`): big-market-activity-service module at port 8090; scan boundary enforced; no draw execution moved; no RPC provider, HTTP controller, MQ consumer, or job handler added; no remote flag introduced. Phase 5-G draw saga/outbox design complete (tag `phase-5-activity-draw-saga-outbox-scaffold`): orchestration saga pattern chosen; IDrawOutboxPort + DrawOutboxEvent + LocalDrawOutboxPort scaffold contracts introduced; design doc committed; IDrawOutboxPort NOT wired into draw hot-path (requires Phase 7-D DDL + Phase 8-E approval); no remote flags introduced. Phase 5 complete. Phase 6-A DAO ownership matrix complete (tag `phase-6-dao-ownership-matrix`): full inventory of 24 DAO interfaces, 23 physical tables, 7 repositories, and 6 cross-boundary access violations documented in `docs/microservices-dao-ownership.md`; no Java files changed; highest-risk violation is StrategyRepository reading activity + quota tables (CRITICAL blocker for Phase 7-A). Phase 6-B package-ownership boundary validator complete (tag `phase-6-package-ownership-boundaries`): 11 cross-boundary violations explicitly allowlisted in `scripts/validate-microservices-phase-6-package-ownership-boundaries.sh`; new DAO coupling outside allowlist fails CI; activity-service scope constraints + Phase 5-D/E/F/G port boundaries verified. Phase 7-A prep (AL-4) complete (tag `phase-7-account-boundary-prep-activity-credit-port`): AL-4 violation (ActivityRepository → IUserCreditAccountDao) resolved by routing credit-account reads through IActivityAccountPort.queryUserCreditAccountAmount; LocalActivityAccountPort delegates to IUserCreditAccountDao; ActivityRepository no longer imports IUserCreditAccountDao; no remote flags enabled; behavior preserved. Phase 7-A prep (AL-2/AL-3) complete (tag `phase-7-account-boundary-prep-strategy-account-port`): AL-2 violation (StrategyRepository → IRaffleActivityAccountDao) and AL-3 violation (StrategyRepository → IRaffleActivityAccountDayDao) resolved by routing reads through IStrategyActivityAccountPort; LocalStrategyActivityAccountPort delegates to IRaffleActivityAccountDao/IRaffleActivityAccountDayDao; StrategyRepository no longer imports either DAO; no remote flags enabled; behavior preserved. Phase 7-A (AL-1) complete (tag `phase-7-strategy-activity-mapping-port`): AL-1 violation (StrategyRepository → IRaffleActivityDao) resolved by routing activityId <-> strategyId mapping reads through IStrategyActivityMappingPort; LocalStrategyActivityMappingPort delegates to IRaffleActivityDao; StrategyRepository no longer imports IRaffleActivityDao directly; no remote flags enabled; behavior preserved; all StrategyRepository cross-boundary DAO couplings are now resolved. Phase 7-B generic task outbox ownership decision complete (tag `phase-7-task-outbox-ownership`): per-domain outbox/task tables chosen for AL-8/AL-9/AL-10, following the existing `credit_award_task` precedent; runtime behavior unchanged and shared `task` couplings remain allowlisted until Phase 7-C+ proposed DDL and Phase 8 cutover gates. Phase 7-A prep (AL-5) complete (tag `phase-7-award-activity-order-boundary`): AwardRepository no longer imports IUserRaffleOrderDao; user_raffle_order create->used transition routes through IAwardActivityOrderPort and LocalAwardActivityOrderPort; no remote flags enabled; transaction and routing behavior preserved. Phase 7-A prep (AL-7) complete (tag `phase-7-credit-award-task-job-boundary`): DispatchCreditAwardTaskJob no longer imports ICreditAwardTaskDao; credit_award_task reads/state transitions route through ICreditAwardTaskDispatchPort and LocalCreditAwardTaskDispatchPort; account.award-credit-outbox.enabled remains default false; behavior preserved.
...
2. `AwardRepository.saveGiveOutPrizesAggregate` writes `user_credit_account` and `user_award_record` in one local transaction (credit-award outbox addresses this but is flag-gated).
...
**Recommended next repo-only batch after AL-7:** prioritize AL-6/AL-11
`AwardRepository` credit outbox write cleanup. That is now the remaining
high-priority account/fulfillment runtime DAO coupling before credit-award
outbox traffic can be safely moved.
...
| task / outbox | shared today; Phase 7-B decision complete | `big-market-domain.task`, `ITaskDao` | per-domain outbox tables: `rebate_task_outbox`, `credit_trade_task_outbox`, `award_dispatch_task_outbox` | `BehaviorRebateRepository`, `CreditRepository`, `AwardRepository`, MQ publish path | `task` (shared until migration) | `SendMessageTaskJob` XXL-Job (in message-job-service) | Phase 7-C proposed DDL or AL-6/AL-11 cleanup | each domain owns its `*_task_outbox`; legacy `task` retained for back-compat until migrated |
