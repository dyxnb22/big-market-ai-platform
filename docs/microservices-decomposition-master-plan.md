...
| task / outbox | shared today; Phase 7-B decision complete | `big-market-domain.task`, `ITaskDao` | per-domain outbox tables: `rebate_task_outbox`, `credit_trade_task_outbox`, `award_dispatch_task_outbox` | `BehaviorRebateRepository`, `CreditRepository`, `AwardRepository`, MQ publish path | `task` (shared until migration) | `SendMessageTaskJob` XXL-Job (in message-job-service) | Phase 7-C proposed DDL | each domain owns its `*_task_outbox`; legacy `task` retained for back-compat until migrated |
