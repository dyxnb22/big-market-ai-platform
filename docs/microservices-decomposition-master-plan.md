...
but actual cutover remains EXTERNAL-GATED. The next execution order is evidence
intake first, then real staging/production evidence, then 7-day legacy-provider
disable decisions, then 30-day obsolete-path removal decisions. Repo-only work
keeps those gates explicit; it does not enable traffic.

| # | Batch | Why this order |
|---|-------|----------------|
| 1 | External evidence intake in `docs/microservices-phase-8-external-evidence-intake.md` | Creates one index for DBA/Ops/Engineering/Oncall/Product evidence while keeping every missing item `EXTERNAL-GATED`. |
| 2 | Staging/prod cutover evidence attachment | Records real external evidence only after staging or production windows run; remote/outbox flags remain default false in repo. |
| 3 | 7-day stable legacy-provider disable decisions | Requires real cutover evidence, a clean 7-day oncall window, and cleanup inventory coverage before any provider is disabled in an environment. |
| 4 | 30-day obsolete-path removal decisions | Requires 30 clean days and explicit signoff before repository deletion of compatibility providers, fallback adapters, mapper copies, or shared task fallbacks. |
| 5 | Keep `scripts/validate-microservices-split-all-gates.sh` green in CI | Aggregates the master plan, Phase 6, Phase 7, Phase 8, service module ownership, production flag, evidence intake, and cleanup gates without Docker/DB/MQ access. |
