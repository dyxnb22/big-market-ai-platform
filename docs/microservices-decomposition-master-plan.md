...
| task / outbox | shared (or message-outbox-service) | Decision deferred to Phase 7 |
...
| 7-B | Generic `task` table strategy decision | docs | Option A: per-domain `*_task_outbox` tables (chosen direction; mirrors `credit_award_task`). Option B: `message-outbox-service` |
...
**7-B** Generic `task` table strategy decision doc. Type: docs. Risk: Medium.
...
| 3 | Shared `task` outbox ownership ambiguity | A second domain inserts into `task` after rebate moves out | Phase 7-B decision doc; per-domain outbox tables follow `credit_award_task` precedent |
