...
**Recommended next repo-only batch after AL-5:** prioritize AL-6/AL-11
`AwardRepository` credit outbox write cleanup if the goal is account/fulfillment
table isolation, or AL-7 `DispatchCreditAwardTaskJob` boundary prep if the next
risk to retire is the message-job-service direct `ICreditAwardTaskDao` import.
