...
- Nacos 3.x: Admin API auth disabled in learning compose (`NACOS_AUTH_ADMIN_ENABLE=false`). Do **not** enable `nacos.config.namespace.compatible.mode`. Config sync is explicitly limited to the public namespace and aligns the live tenant to empty (SDK write target). Publish is fail-closed via JDBC confirmation against empty-tenant MySQL (`nacos.config.sync.confirmJdbcUrl` on admin), locks the empty-tenant source while confirming, and atomically mirrors the payload to the `public` twin that `getConfig` reads. Local snapshots must disable via reflection or admin refuses to start sync; snapshot clear failures also fail-closed. Redis topics fan out runtime/platform updates and zero receivers fail the write. Migrations keep empty as seed SoT and ensure a matching public twin. Learning Docker Dubbo providers disable metadata-report to avoid Nacos 3 metadata noise.
