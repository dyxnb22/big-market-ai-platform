...
| `ICreditAwardTaskDao` | `CreditAwardTask` | `credit_award_task_mapper.xml` | account-service, message-job-service | `credit_award_task` | `LocalAwardCreditWritePort`†§, `LocalCreditAwardTaskDispatchPort`†‡ | fulfillment-service†§, message-job-service†‡ | account-service | 7-A | High | AL-7 and AL-11 resolved via local ports; B18 cutover |
...
§ `AwardRepository` no longer imports `IUserCreditAccountDao` or `ICreditAwardTaskDao` directly. It routes credit-account writes and credit-award outbox inserts through `IAwardCreditWritePort`; `LocalAwardCreditWritePort` owns the local DAO delegation during the dark-launch phase (see §4.3).
...
| `AwardRepository` → `IAwardCreditWritePort` → `IUserCreditAccountDao` | fulfillment | credit | `user_credit_account` | Credit-award default path writes credit account in same local transaction | `saveGiveOutPrizesAggregate` (flag=false default path) | **AL-6 resolved** — direct DAO dependency removed |
| `AwardRepository` → `IAwardCreditWritePort` → `ICreditAwardTaskDao` | fulfillment | credit | `credit_award_task` | Credit-award outbox path inserts task row in same local transaction | `saveGiveOutPrizesAggregate` (flag=true outbox path) | **AL-11 resolved** — direct DAO dependency removed |
...
**AL-6 / AL-11 resolution (Phase 7-A prep):** `IAwardCreditWritePort` added with the exact write operations used by `AwardRepository`: `updateOrCreateCreditAccount(String userId, BigDecimal creditAmount)` for the default local credit-account path and `insertCreditAwardTask(String userId, String awardOrderId, BigDecimal creditAmount)` for the flag-gated outbox path. `AwardRepository.saveGiveOutPrizesAggregate` now delegates both writes to the port rather than injecting `IUserCreditAccountDao` or `ICreditAwardTaskDao` directly. `LocalAwardCreditWritePort` builds the same `UserCreditAccount` request (`userId`, total/available amount, `AccountStatusVO.open`) and the same `CreditAwardTask` fields (`userId`, `awardOrderId`, `creditAmount`), then delegates to the existing DAOs. Behavior is identical: same inputs, same caller-owned Redis lock, same `dbRouter.doRouter(giveOutPrizesAggregate.getUserId())`, same `transactionTemplate.execute` boundaries, same `DuplicateKeyException` handling, and no remote flags enabled.

New boundary paths:

- `AwardRepository` → `IAwardCreditWritePort` → `LocalAwardCreditWritePort` → `IUserCreditAccountDao` (`user_credit_account`)
- `AwardRepository` → `IAwardCreditWritePort` → `LocalAwardCreditWritePort` → `ICreditAwardTaskDao` (`credit_award_task`)

Risk level: **HIGH but reduced** — AL-5, AL-6, and AL-11 direct DAO dependencies are resolved. Runtime account-service table isolation still requires Phase 8-B cutover evidence because the default flag remains `account.award-credit-outbox.enabled=false`; this batch only introduces the local boundary and preserves behavior.
...
4. **AwardRepository local credit DAO access removed** (§4.3): Done — `IAwardCreditWritePort` introduced; `LocalAwardCreditWritePort` holds `IUserCreditAccountDao` and `ICreditAwardTaskDao`; `AwardRepository` no longer imports either DAO. Runtime cutover to making `credit_award_task` the sole credit-write channel still requires Phase 8-B flag enablement + staging evidence.
