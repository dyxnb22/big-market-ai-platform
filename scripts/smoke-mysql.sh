#!/usr/bin/env bash
# API smoke against MySQL backend (skips when MySQL unavailable).
set -euo pipefail
...
if ! command -v mysql >/dev/null 2>&1; then
  echo "SKIP  smoke-mysql: mysql client not installed"
  exit 0
fi

if ! mysql -h"$MYSQL_HOST" -P"$MYSQL_PORT" -u"$MYSQL_USER" -p"$MYSQL_PASS" -e "SELECT 1" >/dev/null 2>&1; then
  echo "SKIP  smoke-mysql: MySQL not reachable at ${MYSQL_HOST}:${MYSQL_PORT}"
  exit 0
fi
